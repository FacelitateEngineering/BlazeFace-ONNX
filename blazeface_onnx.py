...
class BlazeFace(nn.Module):
    ...
    def forward(self, x):
        ...
        results = self.detector.run([output.name for output in self.outputs], {self.inputs[0].name: x})
        c = np.concatenate([results[0], results[1]], axis=1) 
        r = np.concatenate([results[2], results[3]], axis=1)
    ...
    def load_weights(self, path):
        self.detector = onnxruntime.InferenceSession(path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        self.inputs = self.detector.get_inputs()
        self.outputs = self.detector.get_outputs()
